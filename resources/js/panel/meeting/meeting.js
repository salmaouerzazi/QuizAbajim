(function ($) {
        ...
        toTimepicker = $(".to-clockpicker").clockpicker({
            ...
            afterDone: () => {
                ...
                $(".to-time")
                    .removeClass("pulsate")
                    .html(
                        substringText(to_time, am_pm) +
                            ' <span class="js-am-pm font-20">' +
                            am_pm +
                            "</span>",
                    );
    ...
    $("body").on("change", "#timeTwelveSwitch", function (e) {
        ...
        const $fromText = $(".from-time").find(".js-am-pm");
        const $toText = $(".to-time").find(".js-am-pm");
    ...
    $("body").on("click", "#saveTime", function (e) {
        ...
        $.post("/panel/meetings/saveTime", data, function (result) {
            ...
            if (errorContainer.length === 0) {
                errorContainer = $('<div class="error-container text-danger mt-2"></div>');
                $modal.append(errorContainer);
            }
            errorContainer.text(err.responseJSON.error || "An unexpected error occurred.");
        });
    ...
    function deleteTimeModal(time_id) {
        var html =
            ...
            '        <button type="button" id="deleteTime" data-time-id="' +
            time_id +
    ...
    $("body").on("click", "#deleteTime", function (e) {
        ...
        for (let id of time_id) {
            $('.remove-time[data-time-id="' + id + '"]')
                .parent()
                .remove();
        }
    });

        // Prevent click events on delete buttons from bubbling up to parent elements
        $("body").on("click", ".delete-meeting", function (e) {
            e.stopPropagation(); // Ensure no further event handling for this click
        });
    ...
    $("body").on("click", ".clear-all", function (e) {
        ...
        const timeIds = parent
            .find(".selected-time .remove-time")
            .map(function () {
    ...
    $("body").on("change", "#temporaryDisableMeetingsSwitch", function (e) {
        ...
        $.post(
            "/panel/meetings/temporaryDisableMeetings",
            data,
            function (result) {
