    ...
    <div class="row mt-20">
        <div class="col-12 col-lg-4">
           ...
           {{--<div class="form-group">
            ...
            <input type="text" name="iban" value="123456789" class="form-control @error('iban')  is-invalid @enderror" placeholder="" {{ ($user->financial_approval) ? 'disabled' : '' }} hidden/>

            <div class="form-group">
                <label class="input-label">{{ trans('financial.account_id') }}</label>
                <input type="text" name="account_id" maxlength="20" value="{{ (!empty($user) and empty($new_user)) ? $user->account_id : old('account_id') }}" class="form-control @error('account_id')  is-invalid @enderror" placeholder="" {{ ($user->financial_approval) ? 'disabled' : '' }}/>
                @error('account_id')
                <div class="invalid-feedback">
                    {{ $message }}
                </div>
