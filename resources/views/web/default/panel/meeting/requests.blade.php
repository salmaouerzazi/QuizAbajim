                ...
                <div class="col-12 col-lg-5">
                    <div class="row">
                        <div class="col-12 col-lg-4">
                            <div class="form-group">
                                <label class="input-label">{{ trans('public.matiere') }}</label>
                                <select class="form-control" id="matiere_id" name="matiere_id" onchange="this.form.submit()">
                                    <option value="all">{{ trans('public.all_matiere') }}</option>
                                    @foreach ($matiere as $matiere)          
                                            <option value="{{$matiere->id }}" @if (request()->get('by_level') == $matiere->id) selected="selected" @endif>{{ $matiere->name }}</option>
                                    @endforeach
                                </select>
                            </div>
                        </div>
                            ...
                            <div class="row">
                                <div class="col-12 col-lg-12">
                                    <div class="form-group">
                                        ...
                                        <select name="student_id" class="form-control select2 " onchange="this.form.submit()">
                        ...
                        <div class="table-responsive">
                            <table class="table text-center custom-table">
                                ...
                                <tr>
                                    ...
                                    <th class="text-center">{{ trans('public.matiere') }}</th>
                                    <th class="text-center">{{ trans('public.level') }}</th>
                                ...
                                <tbody>
                                @php
                                    // Define the colors you want to use for different materials
                                    $colors = ['#17A2B8','#FFC107', '#DC3545', '#6f42c1', '#fd7e14', '#20c997'];
                                    $materialColors = [];
                                    $colorIndex = 0;
                                @endphp
                                @foreach($reserveMeetings as $ReserveMeeting)
                                         @php
                                            $materialName = $ReserveMeeting->meetingTime->material->name;

                                            // Assign a color to each unique material
                                            if (!isset($materialColors[$materialName])) {
                                                $materialColors[$materialName] = $colors[$colorIndex % count($colors)];
                                                $colorIndex++;
                                            }

                                            $materialColor = $materialColors[$materialName];
                                        @endphp
                                    ...
                                    <tr>
                                        ...
                                        <td class="align-middle">
                                           <div style="background-color: {{ $materialColor }};" class="d-inline-flex align-items-center rounded  py-5 px-15 font-14 font-weight-500">
                                            <span style="color:#ffffff;"  class="font-weight-500">{{ $materialName }}</span>
                                            </div>
                                        </td>
                                        <td class="font-weight-500 align-middle">{{ $ReserveMeeting->meetingTime->level->name }}</td>

                                        <td class="align-middle">
                                            <span class="font-weight-500">{{  trans('public.'. dateTimeFormat($ReserveMeeting->start_at, 'D')) }}</span>
                                        </td>
                                        <td class="align-middle">
                                            <span>{{ dateTimeFormat($ReserveMeeting->start_at, 'M-d-Y') }}</span>
                                        </td>
                                        <td class="align-middle">
