...
<a href="{{ (!empty($checkSequenceContent) and $sequenceContentHasError) ? '#!' : $assignmentUrl }}"
    ...
    class=" d-flex align-items-start p-10 cursor-pointer {{ (!empty($checkSequenceContent) and $sequenceContentHasError) ? 'js-sequence-content-error-modal' : 'tab-item' }} {{ $user->isAdmin() ? 'js-not-access-toast' : '' }}"
    data-type="assignment" data-id="{{ $item->id }}"
    ...
    <div>
        <div class="">
            ...
            @if (empty($itemHistory) or $itemHistory->status == \App\Models\WebinarAssignmentHistory::$notSubmitted)
                <span
                    class="text-danger font-12 d-block">{{ trans('update.assignment_history_status_not_submitted') }}</span>
            @else
