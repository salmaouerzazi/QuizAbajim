                        ...
                        <li data-id="{{ !empty($chapter) ? $chapter->id : '' }}"
                            ...
                            <div class="d-flex align-items-center justify-content-between " role="tab"
                                ...
                                <div class="d-flex align-items-center"
                                    ...
                                    <div class="">
                                        <span
                                            class="font-weight-bold text-dark-blue d-block cursor-pointer">{{ !empty($chapter) ? $chapter->title : trans('public.add_new_chapter') }}</span>
                                        <span class="font-12 text-gray d-block">
                                            {{ !empty($chapter->chapterItems) ? count($chapter->chapterItems) : 0 }}
                                            {{ trans('public.topic') }}
                                            | {{ convertMinutesToHourAndMinute($chapter->getDuration()) }}
                                            {{ trans('public.hr') }}
                                        </span>
                                    </div>
                                </div>
                                ...
                                <div class="d-flex align-items-center">
                                    ...
                                    @if ($chapter->status != \App\Models\WebinarChapter::$chapterActive)
                                        <span
                                            class="disabled-content-badge mr-10">{{ trans('public.disabled') }}</span>
                                    @endif

                                    <div
                                        class="form-group mt-2 d-flex align-items-center justify-content-between js-switch-parent">
                                        <label class="js-switch cursor-pointer"
                                            for="checkAllContentsPassSwitch_{{ $chapter->id }}">{{ trans('update.check_all_contents_pass') }}</label>

                                        <div class="custom-control custom-switch">
                                            <input id="checkAllContentsPassSwitch_{{ $chapter->id }}" type="checkbox"
                                                name="check_all_contents_pass"
                                                class="custom-control-input js-chapter-check-all-contents-pass"
                                                data-chapter-id="{{ $chapter->id }}"
                                                {{ $chapter->status == \App\Models\WebinarChapter::$chapterActive ? 'checked' : '' }}>
                                            <label class="custom-control-label"
                                                for="checkAllContentsPassSwitch_{{ $chapter->id }}"></label>
                                        </div>
                                    </div>

                                    <script>
                                        document.addEventListener('DOMContentLoaded', function() {
                                            const checkAllContentsPassSwitch = document.querySelectorAll('.js-chapter-check-all-contents-pass');

                                            checkAllContentsPassSwitch.forEach(function(switchElement) {
                                                switchElement.addEventListener('change', function() {
                                                    ...
                                                    fetch(`/admin/chapters/${chapterId}/change-status`, {
                                                            method: 'POST',
                                                            headers: {
                                                        ...
                                                        .then(data => {
                                                            if (data.success) {
                                                                location
                                                            .reload(); // Reload the page if the status update is successful
                                                            }
                                                        })
                                                        .catch(error => {
                                                            console.error('Error:', error);
                                                            alert('An error occurred while updating chapter status.');
                                                        });
                                                });
                                            });
                                        });
                                    ...
                                    <div class="btn-group dropdown table-actions">
                                        <button type="button" class="add-course-content-btn mr-10 dropdown-toggle"
                                        ...
                                        <div class="dropdown-menu ">
                                            @if ($webinar->isWebinar())
                                                <button type="button"
                                                    class="js-add-course-content-btn d-block mb-10 btn-transparent"
                                                    data-webinar-id="{{ $webinar->id }}" data-type="session"
                                            ...
                                            <button type="button"
                                                class="js-add-course-content-btn d-block mb-10 btn-transparent"
                                                data-webinar-id="{{ $webinar->id }}" data-type="text_lesson"
                                                data-chapter="{{ !empty($chapter) ? $chapter->id : '' }}">
                                            ...
                                            <button type="button"
                                                class="js-add-course-content-btn d-block mb-10 btn-transparent"
                                                data-webinar-id="{{ $webinar->id }}" data-type="quiz"
                                                data-chapter="{{ !empty($chapter) ? $chapter->id : '' }}">
                                            ...
                                            @if (getFeaturesSettings('webinar_assignment_status'))
                                                <button type="button"
                                                    class="js-add-course-content-btn d-block mb-10 btn-transparent"
                                                    data-webinar-id="{{ $webinar->id }}" data-type="assignment"
                                                    data-chapter="{{ !empty($chapter) ? $chapter->id : '' }}">
                                    ...
                                    <a href="/admin/chapters/{{ $chapter->id }}/delete"
                                    ...
                                    <i class="collapse-chevron-icon feather-chevron-up text-gray"
                                            ...
                                            <ul class="draggable-content-lists draggable-lists-chapter-{{ $chapter->id }}"
                                                ...
                                                @foreach ($chapter->chapterItems as $chapterItem)
                                                    @if ($chapterItem->type == \App\Models\WebinarChapterItem::$chapterSession and !empty($chapterItem->session))
                                                        @include(
                                                            'admin.webinars.create_includes.accordions.session',
                                                            [
                                                    ...
                                                    @elseif($chapterItem->type == \App\Models\WebinarChapterItem::$chapterFile and !empty($chapterItem->file))
                                                        @include(
                                                            'admin.webinars.create_includes.accordions.file',
                                                            [
                                                    ...
                                                    @elseif($chapterItem->type == \App\Models\WebinarChapterItem::$chapterTextLesson and !empty($chapterItem->textLesson))
                                                        @include(
                                                            'admin.webinars.create_includes.accordions.text-lesson',
                                                            [
                                                    ...
                                                    @elseif($chapterItem->type == \App\Models\WebinarChapterItem::$chapterAssignment and !empty($chapterItem->assignment))
                                                        @include(
                                                            'admin.webinars.create_includes.accordions.assignment',
                                                            [
                                                    ...
                                                    @elseif($chapterItem->type == \App\Models\WebinarChapterItem::$chapterQuiz and !empty($chapterItem->quiz))
                                                        @include(
                                                            'admin.webinars.create_includes.accordions.quiz',
                                                            [
