...
function sendNotification($title= null, $message = null , $recieverId , $senderId = null, $sender = 'system', $type = 'single',  $data = [])
{
...
function customTimeShort($datetime) {
    $now = \Carbon\Carbon::now();
    $diff = $now->diffInSeconds($datetime);

    if ($diff < 60) {
        return trans('notification.seconds_ago', ['count' => $diff]);
    } elseif ($diff < 3600) {
        return trans('notification.minutes_ago', ['count' => intdiv($diff, 60)]);
    } elseif ($diff < 86400) {
        return trans('notification.hours_ago', ['count' => intdiv($diff, 3600)]);
    } else {
...
function fromAndToDateFilterResrvation($from, $to, $query, $column = 'start_at', $strToTime = true)
{
    if (!empty($from) and !empty($to)) {
        $from = $strToTime ? strtotime($from) : $from;
        $to = $strToTime ? strtotime($to) : $to;
...
function getTranslateAttributeValue($model, $key, $loca = null)
{
    ...
    try {
        ...
        if (empty($contentLocale) and empty($loca)) {
