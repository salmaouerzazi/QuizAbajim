...
class MeetingController extends Controller
{
    public function reserve(Request $request)
    {
        ...
        $availableForMeetings = $request->input('available_for_meetings', 'on');
        
        // Return error if meetings are disabled and request is to set 'off'
        if ($availableForMeetings === 'off') {
            return response()->json([
                'title' => trans('public.request_failed'),
        ...
        $meetingTime = MeetingTime::where('id', $timeId)->with('meeting')->first();
        if (!$meetingTime || $meetingTime->meeting->disabled) {
            $toastData = [
                            'title' => trans('public.request_failed'),
                            'msg' => trans('meeting.meeting_disabled'),
                            'status' => 'error'
        ...
        // Check if there's an existing reservation that conflicts
        ...
        if ($reserveMeeting && ($reserveMeeting->locked_at || $reserveMeeting->reserved_at)) {
            return response()->json([
                'title' => trans('public.request_failed'),
                'msg' => $reserveMeeting->locked_at ? trans('meeting.locked_time') : trans('meeting.reserved_time'),
                'status' => 'error'
            ]);
        }

        // Handle the hourly amount logic
        ...
        if (!$hourlyAmountResult['status']) {
            return $hourlyAmountResult['result']; // Assuming it returns a JSON response
        }
  ...
  /*  public function reserveEnfant(Request $request)
    {
        $user = auth()->user();
                ...
                if (!empty($meetingTime)) {
                    ...
                    if (!empty($meeting) and !$meeting->disabled) {
                        if (!empty($meeting->amount) and $meeting->amount > 0) {
                            ...
                            return $this->handleFreeMeetingReservationEnfant($user, $meeting, $meetingTime, $day, $selectedMeetingType, $studentCount);
                        }
                    } else {
                        $toastData = [
                            'title' => trans('public.request_failed'),
    ...
    private function handleFreeMeetingReservation($user, $meeting, $meetingTime, $day, $selectedMeetingType, $studentCount)
    {
        ...
        return redirect('/panel/enfant')->with(['toast' => $toastData]);
    }
    private function handleFreeMeetingReservationEnfant($user, $meeting, $meetingTime, $day, $selectedMeetingType, $studentCount)
    {
        $instructorTimezone = $meeting->getTimezone();
        $explodetime = explode('-', $meetingTime->time);
