...
class RegisterController extends Controller
{
    ...
    */
    ...
    public function showRegistrationInstructorForm()
    {
        $seoSettings = getSeoMetas('register');
        $pageTitle = !empty($seoSettings['title']) ? $seoSettings['title'] : trans('site.register_page_title');
        $pageDescription = !empty($seoSettings['description']) ? $seoSettings['description'] : trans('site.register_page_title');
        ...
        return view(getTemplate() . '.auth.register_instructor', $data);
    }


    protected function validatorParent(array $data)
    {


        return Validator::make($data, [
            'mobile' =>'required|nullable|numeric|unique:users',
            'term' => 'required',
    ...
    protected function create(array $data)
    {
        ...
        $user = User::create([
            ...
            'role_id' => Role::getOrganizationRoleId(),//normal user
    ...
    protected function create_instructor(array $data)
    {
        if (!empty($data['mobile']) and !empty($data['country_code'])) {
            $data['mobile'] = ltrim($data['country_code'], '+') . ltrim($data['mobile'], '0');
        }
    ...
    {
        $this->validatorParent($request->all())->validate();
    ...
    public function registerInstructor(RegisterTeacherRequest $request)
    {

        $validated = $request->validated();

        $user = $this->create_instructor($validated);
        ...
        if ($checkConfirmed['status'] == 'send') {
            ...
            return redirect('/Instructor/verification');
